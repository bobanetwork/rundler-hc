    ...
    CGE: CallGasEstimator<UO = UserOperation>,
{
    ...
    async fn estimate_op_gas(
        ...
        let pre_verification_gas = self.estimate_pre_verification_gas(&op, at_price).await?;
...
mod tests {
    ...
    async fn test_calc_pre_verification_input() {
        ...
        let estimation = estimator
            .estimate_pre_verification_gas(&user_op, None)
            .await
            .unwrap();
    ...
    async fn test_estimation_optional_gas_used() {
        ...
        let estimation = estimator
            .estimate_op_gas(optional_op, spoof::state(), None)
            .await
            .unwrap();
    ...
    async fn test_return_provided_limits() {
        ...
        let estimation = estimator
            .estimate_op_gas(optional_op.clone(), spoof::state(), None)
            .await
            .unwrap();
