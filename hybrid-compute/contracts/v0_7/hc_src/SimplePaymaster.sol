...
/// instead operating at a fixed exchange rate with manual top-ups of its ETH pool. Not suitable for production.
///
...
contract SimplePaymaster is BasePaymaster {
    ...
    /// Fixed exchange rate, set manually.
    ...
    /// @param _cachedPrice New exchange rate
    function setPrice(uint256 _cachedPrice) external onlyOwner {
    ...
    function withdrawToken(address to, uint256 amount) external onlyOwner {
        SafeERC20.safeTransfer(tokenAddr, to, amount);
    }
    ...
    returns (bytes memory context, uint256 validationResult) {unchecked {
        ...
        // note: as price is in native-asset-per-token and we want more tokens increasing it means dividing it by markup
            ...
            SafeERC20.safeTransferFrom(tokenAddr, userOp.sender, address(this), tokenAmount);
