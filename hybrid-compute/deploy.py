...
factory_owner = Web3.to_checksum_address(
...
def deploy2(name, cc, salt):
  ...
  preimage = "0xff" + factory_addr[2:] + salt32[2:] + \
      Web3.to_hex(Web3.keccak(hexstr=initcode))[2:]
...
def buy_insurance(contract):
    ...
    receipt = signAndSubmit(w3, transaction, hc1_key)
    ...
    with open('.env', 'r') as env_file:
      ...
      for i, line in enumerate(lines):
        if line.startswith('POLICY_ID='):
          lines[i] = 'POLICY_ID={}'.format(policy_id)
          break
      else:
        lines.append('POLICY_ID={}'.format(policy_id))
